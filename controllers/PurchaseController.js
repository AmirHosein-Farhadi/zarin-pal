...
exports.checkPay = (req, res) => {
  ...
  Purchase.findOne({ Authority, isChecked: false })
    .exec()
    .catch((error) => {
      console.log('Could Not Find Purchase -> ' + error);
    })
    .then((findedPurchase) => {
      if (!findedPurchase || !findedPurchase.Authority) {
        return res.send({
          message:
            'can not find your purchase or your purchase is not suitable',
        });
      }
      Zarinpal.PaymentVerification({ Authority, Amount: findedPurchase.price })
        ...
        .then((zarin) => {
          if (zarin.RefID && !findedPurchase.isPaid) {
            ...
            findedPurchase
              .save()
              .then((findedPurchaseSaved) => findedPurchaseSaved);
          }
...
exports.getAllUnverified = (req, res) => {
...
exports.refund = (req, res) => {
  ...
  Zarinpal.RefundPurchase(authority)
    .catch((error) => console.error(error))
    .then((zarin) => {
      console.error(zarin);
      res.send({ zarin });
